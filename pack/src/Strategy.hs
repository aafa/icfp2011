...
zombiWaves = concat [flushTemplate ++ updateCounter 1 ++ makeZombie 2 | n <- [1..10]] ++ getJSlotUpToTen ++ concat [flushTemplate ++ updateCounter 2 ++ makeZombie 2 | n <- [1..117]] ++ flushTemplate ++ makeZombie 2 ++ firstKill ++ resetCounters ++ zombiWaves

-- stratages
fastAttack = attack 0 where attack n = simpleAttackOn n (n+2) ++ strike 1001 0 n ++ attack (n+1)
zombiAttack = makeZF ++ makeNeutralTemplate tSlot ++ initCounters ++ flushTemplate ++ makeZombie 2 ++ zombiWaves
